...
## This round: logo fix, real parallax, PDF menu, and a QA pass

- **Logo re-cropped properly** — tight circular crop with zero wall background showing, at the correct resolution (was accidentally huge before, ~1.1MB for a 48px icon; fixed).
- **True "stays in place while scrolling" background** — the hero backdrop now uses real CSS `background-attachment: fixed`, the same technique kebabsonmaskell.co.nz uses, not a drifting parallax. A second fixed-background band was added near the bottom of the homepage for the same effect in a second spot. Both gracefully fall back to a normal scrolling background on mobile, since `fixed` attachment is unreliable on touch browsers — this is standard practice, not a compromise.
- **Downloadable PDF menu** (`images/almaz-menu.pdf`, linked from the menu page) — matches the reference site's "Download our menu" feature, generated directly from your real menu data.
- **Testimonial avatars** — small monogram-style avatar circles, matching the reference site's "photo next to quote" testimonial layout. These are generic initials, not real photos of strangers, since I don't have rights to use anyone's actual photo.
